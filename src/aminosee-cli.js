...
function pushCli(cs) {
  ...
  class AminoSeeNoEvil {
    ...
    initStream() {
      ...
      // startStreamingPng();
      let msg = `Streaming ${this.peptide} ${bytes( this.baseChars )} c${ this.codonsPerPixel } m${this.dimension}`
      procTitle( "streaming" )
      mode( msg )
    ...
    setupLinearNames() { // must not be called during creation of hilbert image
      ...
      let msg = `Setup linear names: ${ this.currentFile } highlight set: ${isHighlightSet} peptide: ${this.peptide} focus: ${this.peptide} triplet ${this.triplet}`
      mode(msg)
      bugtxt(msg)
    ...
    fancyFilenames() {
      ...
      output(chalk.rgb(100,100,180).inverse.underline(fixedWidth( this.colDebug*2.5, `HTML: ${ path.normalize( this.fileHTML )}`)))
      log(chalk.white.bgBlue.inverse(        fixedWidth( this.colDebug*2.5,  `Lockfile: ${ path.normalize( this.fileTouch )}`)))
      output(blueWhite(`URL: ${ chalk.underline( this.currentURL )}`))
    }
    ...
    saveDocsSync(cb) {
      ...
      this.saveHilbert(this.saveHTML( () => { this.postRenderPoll(`save docs cb`) } )) // <--- that's some callback hell right there!
    ...
    saveHTML(cb) {
      ...
      if ( this.isHighres() && isPreview ) { mode("not outputting html due to this is preview") ; this.isDiskFinHTML = true }
