...
module.exports = () => {
  ...
  if (isElectron == true) {
    ...
    setupApp();
    addJob( populateArgs( process.argv ) ); // populate args returns the args.
  }
}
...
class AminoSeeNoEvil {
  constructor(classyArgv) { // CLI commands, this.filenames, *
    output(logo());
    this.setupOutPaths();
    setupPrefs(this);
  }
  ...
  setupJob( args ) {
    ...
    const radMessage = terminalRGB(`
      ...
      if ( args.peptide || args.p) {
        ...
        this.peptide = this.tidyPeptideName( this.usersPeptide );
        if ( this.peptide !== "none"  ) { // this colour is a flag for  this.error
          this.isHighlightSet = true;
          output(`User has set highlight mode to ${ this.peptide }`);
        } else {
          log(`Sorry, could not lookup users peptide: ${ this.usersPeptide } using ${ this.peptide }`);
        }
      } else {
    ...
    toggleDevmode() {
      ...
      if (this.devmode == true) {
        ...
        if (this.debug == true) {
          this.raceDelay += 5000; // this helps considerably!
        }
        output("AminoSee has been slowed to " + this.raceDelay)
      } else {
    ...
    pollForStream(reason) { // render lock must be off before calling. aim: start the render, or look for work
      ...
      //   return true;
      // }
      if (this.howMany < 1) { isShuttingDown = true;}
  ...
  lookForWork( reason ) {
    ...
    //   return false;
    // }
    // bugtxt(`Polling in ${ this.raceDelay } ms ${this.howMany} files remain, next is ${maxWidth(32, this.nextFile)}`);
    // if ( !this.renderLock) {
  ...
  initStream() {
    ...
    this.rgbArray = [];
    this.hilbertImage = [];
    bugtxt(`Loading ${ this.filename } Filesize ${bytes( this.baseChars)}`);
    if ( this.clear == true) {
  ...
  highlightFilename() {
    // bugtxt(`Call to highlight filename made`)
    let ret = "";
  ...
  generateFilenameHistogram() {
    this.filenameHistogram = path.normalize( path.resolve(`${ this.outputPath }/${ this.justNameOfDNA}/aminosee_histogram.json`) );
    return this.filenameHistogram;
  }
    ...
    async.series( [
      ...
      function( cb ) {
        if ( that.test ) {
          ...
          that.filenamePNG = that.filenameHILBERT;
          that.rgbArray = that.hilbertImage
          that.ratio = 'sqr'
          that.savePNG( cb );
        } else {
          log('async save png')
  ...
  removeLocks(cb) { // just remove the lock files.
    this.percentComplete = 1;
    mode('remove locks');
    bugtxt('remove locks with ' + this.howMany + ' files in queue. this.filenameTouch: ' + this.filenameTouch)
    this.renderLock = false
    this.howMany--
    clearTimeout( this.updatesTimer);
    clearTimeout( this.progTimer);
  ...
  popAndPollOrBust(reason) { // ironic its now a .shift()
    ...
    if ( file == undefined) {
      mode('filename was undefined after resolve: ' + file)
      // this.quit(0, 'no more commands' );
      return false;
    }
    try {
  ...
  quit(code, reason) {
    ...
    if (code == 0) {
      ...
      if ( this.keyboard ) {
        ...
        log(`If process hangs, remove this line, its preventing a process exit. Reason: ${reason}`)
        return true;
      }
  ...
  calculateShrinkage() { // danger: can change this.filenames of Hilbert images!
    ...
    bugtxt(`filenameHILBERT after shrinking: ${ this.filenameHILBERT } this.dimension: ${ this.dimension }  this.shrinkFactor post ${ twosigbitsTolocale( this.shrinkFactor)} this.codonsPerPixel ${ this.codonsPerPixel } this.codonsPerPixelHILBERT ${ this.codonsPerPixelHILBERT }`);
  }
  ...
  mkdir(relative, cb) { // returns true if a fresh dir was created
    ...
    if ( doesFolderExist(this.outputPath) == false) {
      try {
        fs.mkdirSync(this.outputPath, function (err, result) {
          ...
          // if (err) { bugtxt(`Couldnts create output folder: ${err}`) }
        });
      } catch(e) {
    ...
    this.howMany = 0;
    bugtxt(`this.outputPath: ${ this.outputPath }`);
  ...
  calcUpdate() { // DONT ROUND KEEP PURE NUMBERS
    ...
    let msg = `${ this.nicePercent()} remain ${humanizeDuration( this.timeRemain)} `
    if ( isShuttingDown ) {
      msg += " SHUTTING DOWN "
    } else {
      wTitle(msg);
  ...
  drawHistogram() {
    ...
    if ( this.dnabg  == true) {
      ...
      output(`     To disable real-time DNA background use any of --no-this.dnabg --no-updates --quiet -q  (${termPixels.toLocaleString()})`);
      // }
    }
  ...
  tidyPeptideName(str) { // give it "OPAL" it gives "Opal". GIVE it aspartic_ACID or "gluTAMic acid"
    if (str == undefined) { bugtxt(`error with str it equals ${str} err: [${e}] will return "none"`)
      return "none";
    }
    try {
      str = spaceTo_( str.toUpperCase() )
    } catch(e) {
      bugtxt(`error with str it equals ${str} err: [${e}] will return "none"`)
      return "none";
    }
    for ( let i = 0; i < this.pepTable.length; i++) {
      let compareTo = spaceTo_( this.pepTable[i].Codon.toUpperCase() )
      if ( compareTo == str ) {
        bugtxt(`str  ${str} = ${compareTo}  compareTo: ${this.pepTable[i].Codon} ` + chalk.yellow(`<--  GREAT SUCCESS`))
        return this.pepTable[i].Codon
      } else {
        bugtxt(`str  ${str} = ${compareTo}  compareTo: ${this.pepTable[i].Codon}`)
      }
    }
    return "none";
  }
