...
module.exports = () => {
 ...
 /* invoked on unknown param */
    ...
    boolean: [ 'verbose' ],
    boolean: [ 'devmode'],
    alias: { v: 'verbose', s: 'science', f: 'forceOverwrite', d: 'devmode' },
    */
    ...
    try {
      fs.readFile(filename, function (err, data) {
        ...
        if (!devmode) { // https://nodejs.org/en/docs/guides/event-loop-timers-and-nexttick/
          ...
          output("AminoSee may wait on your browser to close... or try Control-c");
...
function processNewStreamingMethod(f) {
  ...
  if (!parseFileForStream(f)) {
    output("parseFileForStream returned false");
    return false;
  };
  let colorClock = 0;
  percentComplete = 0;
...
function drawHistogram() {
  ...
  text += `
  [ ${Math.round(runningDuration/1000)} s runtime ${percentComplete}% done ] [codons: ${genomeSize.toLocaleString()}] Byte remain: ${Math.round((baseChars - charClock)/1000)/1000} Last Acid: `;
