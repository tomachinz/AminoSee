var assert = require('assert'),
    ...
    httpServer = require('../node_modules/http-server');

var root = path.join(__dirname, 'fixtures', 'root');

vows.describe('http-server').addBatch({
  'When http-server is listening on 44444': {
    topic: function () {
    ...
    'it should serve files from root directory': {
      topic: function () {
        request('http://localhost:44444/rhino.jpg', this.callback);
      },
    ...
    'when requesting non-existent file': {
      topic: function () {
        request('http://localhost:44444/404', this.callback);
      },
    ...
    'when requesting /': {
      topic: function () {
        request('http://localhost:44444/', this.callback);
      },
    ...
    'When http-server is proxying from 8081 to 44444': {
      topic: function () {
        var proxyServer = httpServer.createServer({
          proxy: 'http://localhost:44444/',
      ...
      'it should serve files from the proxy server root directory': {
        topic: function () {
          request('http://localhost:8081/root/file', this.callback);
        },
      ...
      'it should fallback to the proxied server': {
        topic: function () {
          request('http://localhost:8081/file', this.callback);
        },
        'status code should be the endpoint code 200': function (res) {
          assert.equal(res.statusCode, 200);
        },
