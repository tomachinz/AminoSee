...
TITLE="github.com/tomachinz/AminoSee       Audio track is the AminoSee audio logo recording session out-takes"
...
AUDIOTRACK="audio/master2-long.ogg"
...
ENCODE="$FASTCOMMAND -o - | ffmpeg -probesize 63M  -y -r 60 -f image2pipe -vcodec ppm -i - -vcodec libx264  -b:v 1000k -preset slow -pix_fmt yuv420p -crf 5 -threads 0 -bf 0 $FASTOUTPUTFILEAUDIO"
mkdir video
...
ENCODE="$COMMAND -o - | ffmpeg -probesize 63M  -y -r 60 -f image2pipe -vcodec ppm -i - -vcodec libx264  -b:v 1000k -preset slow -pix_fmt yuv420p -crf 5 -threads 0 -bf 0 $OUTPUTFILEMP4"
echo About to run...:
echo $COMMAND
...
ENCODE="$COMMAND -o - | ffmpeg -probesize 100M  -y -r 60 -f image2pipe ppm -i - -vcodec libvpx -acodec libvorbis -b:v 1024k -preset slow -pix_fmt yuv420p -crf 5 -threads 0 -bf 0 $OUTPUTFILEWEBM"
echo $ENCODE
sleep 1
eval $ENCODE
...
# gource --date-format '%d / %m / %Y %a' --camera-mode overview -1100x880 --seconds-per-day 0.2 --auto-skip-seconds 0.1 --max-file-lag 0.01 --background-image src/public/WhitePaper_800px.png --logo src/public/funk-logo-140px.png  --font-size 14 --key --bloom-multiplier 0.125 --bloom-intensity 0.125 --background 012345  -e 0.75 --title 'github.com/tomachinz/AminoSee'
#
...
# $COMMAND -o - | ffmpeg -y -r 60 -f image2pipe -vcodec ppm -i - -vcodec libx264 -preset ultrafast -pix_fmt yuv420p -crf 1 -threads 0 -bf 0 gource.mp4
# $COMMAND -o - | ffmpeg -y -r 60 -f image2pipe -vcodec ppm -i - -vcodec libx264 -preset ultrafast -pix_fmt yuv420p -crf 1 -threads 0 -bf 0 gource.mp4
